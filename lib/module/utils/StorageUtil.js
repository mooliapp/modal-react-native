...
export const StorageUtil = {
  ...
  setDeepLinkWallet(link) {
    return AsyncStorage.setItem(StorageUtil.WALLETCONNECT_DEEPLINK_CHOICE, JSON.stringify({
      href: link
    }));
  },
  ...
  setRecentWallet(wallet) {
    try {
      AsyncStorage.setItem(StorageUtil.W3M_RECENT_WALLET_INFO, JSON.stringify(wallet));
    } catch (error) {
  ...
  async getRecentWallet() {
    try {
      const wallet = await AsyncStorage.getItem(StorageUtil.W3M_RECENT_WALLET_INFO);
